...
if [ -z "$SKIP_DB" ]; then
	bake -s RunMySql path=data randomPort=true notInteractive=true | iconv -f cp866 -t cp1251 ; test ${PIPESTATUS[0]} -eq 0
	port=$(cat data/port)
...
then
	...
	bake -s packages:install notInteractive=true | iconv -f cp866 -t cp1251 ; test ${PIPESTATUS[0]} -eq 0
	if [ $? -ne 0 ]; then
		exit $?
	fi
	bake -s fix:packages | iconv -f cp866 -t cp1251 ; test ${PIPESTATUS[0]} -eq 0
fi
bake TryToBuild Port=$port notInteractive=true | iconv -f cp866 -t cp1251 ; test ${PIPESTATUS[0]} -eq 0
if [ -z "$SKIP_DB" ]; then
	bake db:setup Port=$port notInteractive=true | iconv -f cp866 -t cp1251 ; test ${PIPESTATUS[0]} -eq 0
fi
bake generate:binding:redirection notInteractive=true
bake test Port=$port notInteractive=true | iconv -f cp866 -t cp1251 ; test ${PIPESTATUS[0]} -eq 0
