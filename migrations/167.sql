create table Future.OrderSwap(
  SupplierId int unsigned not null,
  ClientId int unsigned not null,
  SwapFirmCode tinyint(1) not null default 0
);

DROP PROCEDURE IF EXISTS `GetOrderHeader`;
CREATE DEFINER=`RootDBMS`@`127.0.0.1` PROCEDURE `GetOrderHeader`(IN idparam  integer unsigned)
BEGIN
