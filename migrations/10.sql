drop procedure IF EXISTS logs.ClearStatistics;

CREATE DEFINER = 'RootDBMS'@'127.0.0.1'
PROCEDURE logs.ClearStatistics()
BEGIN
  DELETE
  FROM
    AmpLogs
...
TRIGGER farm.FormRulesLogDelete
AFTER DELETE
ON farm.formrules
FOR EACH ROW
BEGIN
...
CREATE
DEFINER = 'RootDBMS'@'127.0.0.1'
TRIGGER farm.FormRulesLogInsert
AFTER INSERT
ON farm.formrules
FOR EACH ROW
BEGIN
...
CREATE
DEFINER = 'RootDBMS'@'127.0.0.1'
TRIGGER farm.FormRulesLogUpdate
AFTER UPDATE
ON farm.formrules
FOR EACH ROW
BEGIN
