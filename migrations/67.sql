...
-- Правка триггеров для таблицы farm.FormRules. Создаем триггеры без вставки в поле ParentFormRules

DROP TRIGGER IF EXISTS farm.FormRulesLogInsert;


CREATE DEFINER = RootDBMS@127.0.0.1 TRIGGER farm.FormRulesLogInsert AFTER INSERT
ON farm.FormRules
FOR EACH ROW BEGIN
  INSERT
...
CREATE DEFINER = RootDBMS@127.0.0.1 TRIGGER farm.FormRulesLogUpdate AFTER UPDATE
ON farm.FormRules
FOR EACH ROW
BEGIN
  INSERT
...
CREATE DEFINER = RootDBMS@127.0.0.1 TRIGGER farm.FormRulesLogDelete AFTER DELETE
ON farm.FormRules	
FOR EACH ROW BEGIN
  INSERT
  INTO `logs`.form_rules_logs
